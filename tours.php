            ...
            <div class="row">
                <?php
                    if(isset($_POST['sDate'])) {
                        $stime = new DateTime($_POST['sDate']);
                        $etime = new DateTime($_POST['eDate']);
                        $result = $conn->query("SELECT * FROM `event` WHERE `start_date` >= '" . $stime->getTimestamp() . "' and `end_date` <= '" . $etime->getTimestamp() . "' ORDER BY `e_id` DESC ");
                    }else{
                        $result = $conn->query("SELECT * FROM `event`  ORDER BY `e_id` DESC ");
                    }
                    if($result->num_rows > 0) {
                        while($row=$result->fetch_assoc()) {
                            ?>
                            <div class="col-md-6 col-lg-6 mb-4 ftco-animate">
                                <a href="tours_info.php?id=<?php echo $row['e_id'] ?>" class="block-5" style="background-image: url('<?php echo $row['image'] ?>');">
                                    <div class="text">
                                        <span class="price">৳<?php echo $row['cost'] ?></span>
                                        <h3 class="heading"><?php echo $row['e_title'] ?></h3>
                                        <div class="post-meta">

                                            <span><b><?=date('d M, Y',$row['start_date']); ?></b> to <b><?=date('d M, Y',$row['end_date']); ?></b></span>
                                        </div>

                                        <?php
                                        $res = $conn->query("SELECT count(r_id) as `count`, sum(r_value) as `total` FROM `rate` WHERE `e_id` = '".$row['e_id']."'");
                                        $rows = $res->fetch_assoc();
                                            ...
                                            if($rows['count'] != 0)
                                                ...
                                                $rate = 0;
                                            ?>
                                            <div class='rating-stars star-rate' style="font-size: 4px; color: white;">
                                                <ul>
                                                    <?php
                                                    for($i = 0; $i < 5; $i+=1){
                                                    ...
                                                    ?>
                                                    <li class="star"><span class="review"><?=$rows['count'];?> reviews</span></li>
                                                </ul>
          ...
          <div class="col-lg-4 sidebar">
              ...
              <div class="sidebar-box ftco-animate">
                  <div class="search-tours bg-light p-4">
                      <form action="create_tour.php">
                          <div class="fields">
                              <div class="row flex-column">
                                  <div class="col-sm-12 group mb-3">
                                      <input type="submit" class="search-submit btn btn-primary" value="Create New Tour">
                                  </div>
                              </div>
                          </div>
                      </form>
